pipeline {
    ...
    stages {
        ...
        stage('Build App') {
            steps {
                sh './gradlew clean --refresh-dependencies --no-daemon :gallery-app:build -PJFROG_CREDENTIALS_USR=$JFROG_CREDENTIALS_USR -PJFROG_CREDENTIALS_PSW=$JFROG_CREDENTIALS_PSW'
            }
        }
        stage('Publish App') {
            steps {
